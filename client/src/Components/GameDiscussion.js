...
export var interaction_msgs = [];

function GameDiscussion({
  question,
  currentRound,
  funccompletediscussion,
  funcToGameStart,
}) {
      ...
      if (event.data instanceof Blob) {
        ...
        reader.onload = function () {
          if (reader.result) {
            setMessages((prev) => [
              ...prev,
              { text: reader.result.toString(), type: "received" },
            ]);
            interaction_msgs.push({ text: reader.result.toString(), type: "received" });
          }
        };
        ...
        interaction_msgs.push({ text: event.data, type: "received" });
      }
    };
    setWs(newWs);
    return () => newWs.close();
  ...
  const sendMessage = () => {
    ...
    if (ws && ws.readyState === ws.OPEN && input) {
      ...
      interaction_msgs.push({ text: input, type: "sent" });
      setInput("");
    }
  };
    ...
    <div>
      <div className="App1">
        <div className="QuestionInfo">
          ...
          <p className="Source">
            <b>Source:</b> <a href={question.source}>Link</a>
          </p>
        </div>
        <div className="Chat2">
          <div className="starter-questions">
            ...
            <p>
              What did you find most challenging about avoiding the taboo words
              in this round?
            </p>
      ...
      <div className="next">
        ...
        {currentRound < 5 ? (
          <button className="nextQuestion" onClick={funcToGameStart}>
            Next Question
          </button>
        ) : (
          <button className="survey" onClick={funccompletediscussion}>
            Survey
          </button>
        )}
      </div>
